        ...
        for (int i=1; i<=testCases.size(); i++) {
            try {
                ...
                Future<Answer> future = executorService.submit(() -> {
                     Q3Object input =
                            new ObjectMapper().configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false).readValue(testCase.getInput(),
                                    Q3Object.class);
                    long startTime = System.nanoTime();
                    Integer answer = skeleton.answers.Question3.lowestExposureToExchanges(input.numNodes,
                            input.edges);
