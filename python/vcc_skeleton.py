...
class Test(unittest.TestCase):
    ...
    def test_runq1_main(self):
        ...
        if travis_uuid != '' or None:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/1/" + travis_uuid
        else:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/1/"
        ...
        if travis_uuid != '' or None:
            ...
            requests.post('https://ta-cc-gl.herokuapp.com/answer/contestant/' + travis_uuid + '/1',
                          data=jsonresponse, headers=headers)

    def test_runq2_main(self):
        travis_uuid = os.environ.get('travistestidentifier', '')
        if travis_uuid != '' or None:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/2/" + travis_uuid
        else:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/2/"
        tests = requests.get(url=url).json()
        response = []
        for testnumber in range(0, len(tests)):
            try:
        ...
        if travis_uuid != '' or None:
            jsonresponse = json.dumps(response)
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
            requests.post('https://ta-cc-gl.herokuapp.com/answer/contestant/' + travis_uuid + '/2',
                          data=jsonresponse, headers=headers)

    def test_runq3_main(self):
        travis_uuid = os.environ.get('travistestidentifier', '')
        if travis_uuid != '' or None:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/3/" + travis_uuid
        else:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/3/"
        tests = requests.get(url=url).json()
        response = []
        for testnumber in range(0, len(tests)):
            try:
        ...
        if travis_uuid != '' or None:
            jsonresponse = json.dumps(response)
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
            requests.post('https://ta-cc-gl.herokuapp.com/answer/contestant/' + travis_uuid + '/3',
                          data=jsonresponse, headers=headers)

    def test_runq4_main(self):
        travis_uuid = os.environ.get('travistestidentifier', '')
        if travis_uuid != '' or None:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/4/" + travis_uuid
        else:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/4/"
        tests = requests.get(url=url).json()
        response = []
        for testnumber in range(0, len(tests)):
            try:
        ...
        if travis_uuid != '' or None:
            ...
            print(jsonresponse)
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
            requests.post('https://ta-cc-gl.herokuapp.com/answer/contestant/' + travis_uuid + '/4',
                          data=jsonresponse, headers=headers)

    def test_runq5_main(self):
        travis_uuid = os.environ.get('travistestidentifier', '')
        if travis_uuid != '' or None:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/5/" + travis_uuid
        else:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/5/"
        tests = requests.get(url=url).json()
        response = []
        for testnumber in range(0, len(tests)):
            try:
        ...
        if travis_uuid != '' or None:
            jsonresponse = json.dumps(response)
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
            requests.post('https://ta-cc-gl.herokuapp.com/answer/contestant/' + travis_uuid + '/5',
                          data=jsonresponse, headers=headers)

    def test_runq6_main(self):
        travis_uuid = os.environ.get('travistestidentifier', '')
        if travis_uuid != '' or None:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/6/" + travis_uuid
        else:
            url = "https://ta-cc-gl.herokuapp.com/tests/run/6/"
        tests = requests.get(url=url).json()
        response = []
        for testnumber in range(0, len(tests)):
            try:
        ...
        if travis_uuid != '' or None:
            jsonresponse = json.dumps(response)
            headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
            requests.post('https://ta-cc-gl.herokuapp.com/answer/contestant/' + travis_uuid + '/6',
                          data=jsonresponse, headers=headers)
...
def runq2(i, return_dict):
    start = clock()
    output = question02(i['cashFlowIn'], i['cashFlowOut'])
    end = clock()
    diff = end - start
    return_dict['output'] = output
    return_dict['diff'] = diff
...
def runq3(i, return_dict):
    start = clock()
    output = question03(i['numNodes'], i['edges'])
    end = clock()
    diff = end - start
    return_dict['output'] = output
    return_dict['diff'] = diff
...
def runq4(i, return_dict):
    start = clock()
    output = question04(i['rows'], i['numberMachines'])
    end = clock()
    diff = end - start
    return_dict['output'] = output
    return_dict['diff'] = diff
...
def runq5(i, return_dict):
    start = clock()
    output = question05(i['allowedAllocations'], i['totalValue'])
    end = clock()
    diff = end - start
    return_dict['output'] = output
    return_dict['diff'] = diff
...
def runq6(i, return_dict):
    start = clock()
    output = question06(i['numServers'], i['targetNode'], i['arcs'])
    end = clock()
    diff = end - start
    return_dict['output'] = output
    return_dict['diff'] = diff
